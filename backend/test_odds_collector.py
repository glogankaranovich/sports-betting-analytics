...
from odds_collector import OddsCollector, convert_floats_to_decimal, lambda_handler, get_secret
...
class TestOddsCollector(unittest.TestCase):
    ...
    def test_init_with_secret_arn(self, mock_get_secret, mock_boto3):
        ...
        with patch.dict('os.environ', {'ODDS_API_SECRET_ARN': 'arn:aws:secretsmanager:us-east-1:123:secret:test', 'DYNAMODB_TABLE': 'test-table'}):
            collector = OddsCollector()
            
        self.assertEqual(collector.api_key, 'test-api-key')
        mock_get_secret.assert_called_once_with('arn:aws:secretsmanager:us-east-1:123:secret:test')
    
    @patch('odds_collector.boto3.resource')
    def test_init_with_env_key(self, mock_boto3):
        mock_boto3.return_value.Table.return_value = self.mock_table
        
        with patch.dict('os.environ', {'ODDS_API_KEY': 'env-api-key', 'DYNAMODB_TABLE': 'test-table'}):
            collector = OddsCollector()
    ...
    def test_get_active_sports(self, mock_boto3, mock_requests):
        ...
        with patch.dict('os.environ', {'ODDS_API_KEY': 'test-key', 'DYNAMODB_TABLE': 'test-table'}):
            collector = OddsCollector()
            result = collector.get_active_sports()
        
        self.assertEqual(result, ['americanfootball_nfl', 'basketball_nba'])
...
class TestLambdaHandler(unittest.TestCase):
    ...
    def test_lambda_handler_success(self, mock_collector_class):
        ...
        self.assertIn('Successfully collected odds for 5 games', body['message'])
