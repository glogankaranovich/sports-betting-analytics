...
class PredictionTracker:
    ...
    def generate_game_predictions_for_sport(self, sport: str, model: str = 'consensus', limit: int = None) -> int:
        ...
        for game_data in games_by_id.values():
            try:
                print(f"Processing {sport} game: {game_data['home_team']} vs {game_data['away_team']}")
                prediction = self.analyzer.analyze_game(game_data)
    ...
    def generate_prop_predictions_for_sport(self, sport: str, model: str = 'consensus', limit: int = None) -> int:
        ...
        if limit and limit > 0:
            ...
            print(f"Limited to {len(player_props)} player props for prediction generation")
        ...
        for prop_pred in prop_predictions:
            try:
                ...
                print(f"Error processing prop prediction for {prop_pred.player_name}: {str(e)}")
                continue
