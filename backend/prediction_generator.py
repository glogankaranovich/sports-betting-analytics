...
def lambda_handler(event, context):
    ...
    try:
        ...
        if bet_type == 'games':
            if sport:
                predictions_count = tracker.generate_game_predictions_for_sport(sport, model, limit=limit)
            else:
                predictions_count = tracker.generate_game_predictions(model, limit=limit)
        elif bet_type == 'props':
            if sport:
                predictions_count = tracker.generate_prop_predictions_for_sport(sport, model, limit=limit)
            else:
                predictions_count = tracker.generate_prop_predictions(model, limit=limit)
        else:
            return {
                'statusCode': 400,
                'body': json.dumps({'error': f'Invalid bet_type: {bet_type}. Must be "games" or "props"'})
            }
