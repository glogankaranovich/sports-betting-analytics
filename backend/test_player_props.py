...
class TestPlayerPropsUnit(unittest.TestCase):
    ...
    def test_store_player_props(self, mock_boto3):
        ...
        collector.store_player_props('americanfootball_nfl', 'test_event_123', props_data)
        ...
        # Check key structure
        key = call_args[1]['Key']
        self.assertTrue(key['pk'].startswith('PROP#test_event_123#draftkings#player_pass_tds#Josh Allen'))
...
class TestPlayerPropsIntegration(unittest.TestCase):
    ...
    def test_player_props_filtering_by_player(self):
        ...
        response = requests.get(
            f'{self.api_url}/player-props?player=Allen',
            headers=self.headers
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_player_props_pagination(self):
        ...
        response = requests.get(
            f'{self.api_url}/player-props?limit=5',
            headers=self.headers
        )
        self.assertEqual(response.status_code, 200)
...
if __name__ == '__main__':
    ...
    if '--integration' in sys.argv:
        ...
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestPlayerPropsDataStructure))
        unittest.TextTestRunner(verbosity=2).run(suite)
